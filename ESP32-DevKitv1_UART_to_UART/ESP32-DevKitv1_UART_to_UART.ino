...
void setup() {
  ...
  Serial1.println("U2UXD - Serial TXD is on pin: "+String(TXD2)); 
  Serial1.println("U2UXD - Serial RXD is on pin: "+String(RXD2)); 
}

void loop() {
  // Leemos puerto 2 y enviamos al purto 0:
  if (Serial1.available()) {
    int inByte = Serial1.read();
    Serial.write(inByte);
  }

  // Leemos puerto 0 y enviamos al purto 2:
  if (Serial.available()) {
